 ...
 *
 * You can currently set the number of iterations, the input dimension, the hidden layer size and the learning rate. It currently uses binary Cross-Entropy and simple gradient descent.
 * Currently most things are hardcoded. The calculations are done using Eigen float Matrices.
 * Used inspiration from https://towardsdatascience.com/how-to-build-a-simple-neural-network-from-scratch-with-python-9f011896d2f3 to get the backpropagation working.
 */
class NN {
public:
    /**
    ...
    */
     ...
     */
    float cost_func(const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> &NN_result,
                    const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> &y_output);

    /**
     * The backpropagation of the Neural Network.
     *
     * The general structure of the backpropagation was taken from https://towardsdatascience.com/how-to-build-a-simple-neural-network-from-scratch-with-python-9f011896d2f3 .
     *
     ...
     * @param o_o_layer The output of the output layer with the given inputs and the output of the hidden layer.
     ...
     */
    void backprop(const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> &x_input,
                  const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> &y_output,
    ...
    /**
     * The compute layer computes the output of one of the NN layers with all its neurons.
     *
     * It takes the input to the layer, the weights, the bias and returns the output of the layer without the activation.
    ...
    compute_layer(const Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> &input,
     ...
     *
     * There are currently no sophisticated training algorithms. Just a number of iterations. This functions connects the other functions and also performs the activation.
     * Also does a very weak proof the function.
     ...
     */
    void train_net(int iterations, Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> &x_input,
