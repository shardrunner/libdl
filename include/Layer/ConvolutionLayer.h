...
public:
   ...
   *
   * The filters of the convolution layer are the only custom matrix, that is
   * stored "transposed".
   ...
   */
  ConvolutionLayer(int input_height, int input_width, int input_channels,
   ...
   * Instead of a classic convolution, the matrices are reshaped and a matrix
   * multiplication is performed. dC_dw=conv(a_prev, (dC_da coeffmult* da_dz)
  ...
  /**
   * Transforms input matrix to a im2col matrix for matrix multiplication with
   * he kernel.
   *
   ...
   */
  [[nodiscard]] std::unique_ptr<Eigen::MatrixXf>
  im2col_switched(const Eigen::MatrixXf &input_matrix, int img_height,
                  int img_width, int number_img_channels, int filter_height,
                  int filter_width, int stride, int padding) const;
   ...
   *
   * Compute backpropagation of bias by summing over the channels of all samples
   * and normalizing by dividing by the number of samples.
   * @param dC_dz dC/dz
   */
   ...
   * Depending on the number of dilations 0 are added between rows and columns
   * of the input matrix.
   ...
   * @param dilation The number of dilation elements to add between rows and
   * columns.
