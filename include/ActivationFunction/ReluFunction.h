 ...
 * undesirable behaviour in the form of dying gradients. To circumvent this issue
 * LeakyRelu has a value a bit smaller than 0 for values smaller than 0.
 *
 */
...
public:
   ...
   * [See abstract base class](@ref ActivationFunction)
     ...
     *
     *Apply an unary expression corresponding to a Relu lambda to every matrix element if elem <0 -> 0 else -> elem
   */
   ...
   * [See abstract base class](@ref ActivationFunction)
     *
     *Apply an unary expression corresponding to a derivative relu lambda to every matrix element and multiply with derivative next layer if elem >0 ->1 else -> -elem*leak_factor (leak_factor 0 by default)
   */
