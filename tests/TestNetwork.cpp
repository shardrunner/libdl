...
SCENARIO("Test complete network") {
  GIVEN("A deterministic network architecture") {
    ...
    input.col(0) << 1, 0, -3, 4, 0, 3, 4, 1, 0, 0, 0, 2, 4, 5, 2, 3, -4, 0, 0,
        0, -4, -5, 3, 1, 0, -3, 0;
    input.col(1) << 0, 0, -4, -5, 3, 1, 0, -3, 0, 1, 2, 1, 0, 0, 3, -2, 0, 4, 0,
        3, 4, 1, 0, 0, 0, 2, 4;
    ...
    WHEN("The network is trained") {
      ...
      THEN("The loss should be predictable") {
        REQUIRE(std::abs(mnet.get_loss(mnet.test_network(input), labels) -
                         0.403376) < 0.0001);
      }
    }
  }
