...
void Adam::optimize_weights(Eigen::MatrixXf &values, const Eigen::MatrixXf &derivatives) {
    assert(values.rows() == m_mt.rows() && values.cols() == m_mt.cols() && derivatives.rows() == m_mt.rows() &&
           derivatives.cols() == m_mt.cols() && "Defined input and real input dimensions do not match!");
    ...
    values.array() -= m_learning_rate * m_mt_corrected.array() / (m_vt_corrected.array().sqrt() + m_epsilon);
}

Adam::Adam(int filter_height, int filter_width, int bias_size) : Adam(0.001, 1e-8, 0.9, 0.999,
                                                                      filter_height, filter_width,
                                                                      bias_size) {}

Adam::Adam(float learning_rate, float epsilon, float beta_1, float beta_2, int filter_height, int filter_width,
           int bias_size)
        : m_learning_rate(learning_rate), m_epsilon(epsilon), m_beta_1(beta_1), m_beta_2(beta_2) {
...
void Adam::optimize_bias(Eigen::VectorXf &values, const Eigen::VectorXf &derivatives) {
    assert(values.rows() == m_mt_bias.rows() && values.cols() == m_mt_bias.cols() &&
           derivatives.rows() == m_mt_bias.rows() && derivatives.cols() == m_mt_bias.cols() &&
