...
//  spdlog::debug("Dimension label x: {}, y: {}", label.rows(),label.cols());
  ...
  auto right_side = ((1 - row_label.array()).matrix()).cwiseProduct(((1 - a_prev.array()).log()).matrix());
...
//
  ...
  temp_loss= (label_row.array() == 0).select((float(1) -
                                       a_prev.array()).cwiseInverse(),
                                      -a_prev.cwiseInverse()).matrix();

  //temp_loss.array() = (label_row.array() < 0.5).select((1 - a_prev.array()).cwiseInverse(),-a_prev.cwiseInverse());
}
