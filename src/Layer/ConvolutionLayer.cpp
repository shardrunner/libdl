...
ConvolutionLayer::ConvolutionLayer(int input_height, int input_width, int input_channels, int filter_height,
                                   int filter_width,
    ...
    if (m_input_height < 1 || m_input_width < 1 || m_filter_height < 1 || m_filter_width < 1 || m_input_channels < 1 ||
        m_output_channels < 1) {
...
void ConvolutionLayer::feed_forward(const Eigen::MatrixXf &input) {
    ...
    m_a = *reshape_im2col_result((m_w * (*im2col_input)).transpose(), m_input_height, m_input_width, m_filter_height,
                                 m_filter_width,
...
void ConvolutionLayer::backpropagate_bias(const Eigen::MatrixXf &dC_dz) {
    for (int i = 0; i < m_output_channels; i++) {
        m_dC_db(i) = (float) (dC_dz.block(m_output_size * i, 0, m_output_size, dC_dz.cols()).sum() /
                              ((double) dC_dz.cols()));
    }
}

void
ConvolutionLayer::backpropagate_weights(const Eigen::MatrixXf &a_prev, const Eigen::MatrixXf &dC_dz, int dC_dz_height,
                                        int dC_dz_width) {
    ...
    auto dC_dz_im2col2 = im2col_switched(dC_dz, dC_dz_height, dC_dz_width, m_output_channels, dC_dz_height, dC_dz_width,
                                         1, m_padding);
    auto a_prev_im2col2 = im2col_switched(a_prev, m_input_height, m_input_width, m_input_channels, dC_dz_height,
                                          dC_dz_width, 1, m_padding);

    //dC/dw=Conv(a_prev,dC/dz) and normalization
    m_dC_dw.noalias() = ((a_prev_im2col2->transpose() * (*dC_dz_im2col2)) / dC_dz.cols()).transpose();
}

void ConvolutionLayer::backpropagate_input(const Eigen::MatrixXf &dC_dz, int dC_dz_height, int dC_dz_width) {
    auto flipped_filter = im2col_switched(flip_filter()->transpose(), m_filter_height, m_filter_width, m_input_channels,
    ...
    auto dC_dz_padded_im2col = im2col(*dC_dz_padded, dC_dz_padded_height, dC_dz_padded_width, m_output_channels,
                                      m_filter_height, m_filter_width, 1, m_padding);

    m_dC_da_prev.noalias() = *reshape_im2col_result(dC_dz_padded_im2col->transpose() * (*flipped_filter),
                                                    dC_dz_padded_height, dC_dz_padded_width, m_filter_height,
                                                    m_filter_width, m_input_channels, 1, m_padding, dC_dz.cols());
}

std::unique_ptr<Eigen::MatrixXf>
ConvolutionLayer::im2col(const Eigen::MatrixXf &input_matrix, int img_height, int img_width,
...
std::unique_ptr<Eigen::MatrixXf>
ConvolutionLayer::im2col_switched(const Eigen::MatrixXf &input_matrix, int img_height, int img_width,
                                  int number_img_channels, int filter_height, int filter_width, int stride,
                                  int padding) const {
    m_convlayer_logger->info("Start im2col");
    m_convlayer_logger->debug(
...
int ConvolutionLayer::row_filter_positions(int img_width, int filter_width, int stride, int padding) const {
    return (img_width - filter_width + 2 * padding) / stride + 1;
}

int ConvolutionLayer::col_filter_positions(int img_height, int filter_height, int stride, int padding) const {
    return (img_height - filter_height + 2 * padding) / stride + 1;
}

std::unique_ptr<Eigen::MatrixXf>
ConvolutionLayer::reshape_im2col_result(const Eigen::MatrixXf &input, int input_height, int input_width,
...
std::unique_ptr<Eigen::MatrixXf>
ConvolutionLayer::pad_matrix(const Eigen::MatrixXf &input, int img_height, int img_width, int number_channels,
                             int padding) const {
    m_convlayer_logger->info("Start padding matrix");
    m_convlayer_logger->debug(
            "{} size input matrix; {} rows input matrix; {} cols input matrix; {} img height; {} img width; {} number_channels; {} size padding",
            input.size(), input.rows(), input.cols(), img_height, img_width, number_channels, padding);
...
#pragma omp parallel for
    for (int i = 0; i < number_channels; i++) {
        for (int j = 0; j < img_width; j++) {
            ...
            input_padded->block(img_height_padded * padding + img_size_padded * i + img_height_padded * j + padding, 0,
                                img_height, input.cols()) = input.block(img_size * i + img_height * j, 0, img_height,
                                                                        input.cols());
        }
    }
...
std::unique_ptr<Eigen::MatrixXf>
ConvolutionLayer::dilate_matrix(const Eigen::MatrixXf &input, int img_height, int img_width, int img_channels,
                                int dilation) const {
            ...
            for (int j = 0; j < img_width; j++) {
                for (int k = 0; k < img_height; k++) {
                    input_padded->operator()(
                            i * img_size_dilated + j * img_height_dilated * (dilation + 1) + k * (dilation + 1),
...
std::unique_ptr<Eigen::MatrixXf> ConvolutionLayer::flip_filter() const {
    ...
    for (int i = 0; i < m_input_channels; i++) {
        flipped_filter->block(0, i * m_filter_height * m_filter_width, m_w.rows(),
                              m_filter_height * m_filter_width) = m_w.block(0, i * m_filter_height * m_filter_width,
                                                                            m_w.rows(), m_filter_height *
                                                                                        m_filter_width).rowwise().reverse();
    }
...
void
ConvolutionLayer::set_weights(const Eigen::MatrixXf &new_filter) {
...
const Eigen::VectorXf &ConvolutionLayer::get_bias() const {
    return m_b;
}
...
const Eigen::MatrixXf &ConvolutionLayer::get_weights() const {
    return m_w;
}
