...
Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic> NN::compute_layer(
...
float NN::cost_func(
  ...
  auto right_side = ((1 - y_output.array()).matrix())
                        .cwiseProduct(((1 - NN_result.array()).log()).matrix());
...
void NN::train_net(
  ...
  h_layer << 0.680375, -0.211234, 0.566198, 0.59688, 0.823295, -0.604897,
      -0.329554, 0.536459;
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    backprop(x_input, y_output, h_res_temp, o_res_temp, h_layer, o_layer,
             h_bias, o_bias, learning_rate);
  ...
  /*
      ...
      for (int i=0; i<4; i++) {
          auto h_res_temp = compute_layer(test_x.block(0,i,2,1), h_layer,
     h_bias); h_res_temp = Eigen::tanh(h_res_temp.array()).matrix(); auto
     o_res_temp = compute_layer(h_res_temp, o_layer, o_bias);
          sigmoid(o_res_temp);
          std::cout << "Result for: "<<test_x.block(0,i,2,1).transpose()<< " ->
     predicted: " << lambda(o_res_temp(0,0)) << std::endl;
      }*/
}