...
void ManageLoggers::initLoggers() const {
    try {
        ...
        auto nn_logger = std::make_shared<spdlog::async_logger>("nn", begin(sinks), end(sinks),spdlog::thread_pool(),spdlog::async_overflow_policy::block);
        auto convlayer_logger=std::make_shared<spdlog::async_logger>("convlayer", begin(sinks), end(sinks),spdlog::thread_pool(),spdlog::async_overflow_policy::block);
