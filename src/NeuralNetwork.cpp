...
void NeuralNetwork::train_network(const Eigen::MatrixXf &input,
    ...
    m_nn_logger->debug(
            ...
            input.size(), input.rows(), input.cols(), batch_size, epochs);
    ...
    for (int i = 0; i < epochs; i++) {
        for (int j = 0; j < num_batches; j++) {
            if (num_batches != 1) {
                ...
                label_batch =
                        perm_label.segment(j * batch_size, batch_size);
            } else {
                input_batch = input;
                label_batch = label;
            }
            ...
            if (divisor >= 0) {
                if (j % divisor == 0) {
                    auto feed_forward_res = m_layer_list[m_layer_list.size() - 1]->get_forward_output();
                    auto loss = m_loss_function->calculate_loss(feed_forward_res, label_batch);
...
Eigen::VectorXi NeuralNetwork::calculate_accuracy(const Eigen::MatrixXf &input,
                                                  const Eigen::VectorXi &label) {
    feed_forward(input);
...
void NeuralNetwork::add_conv_layer(int input_height, int input_width, int input_channels, int filter_height,
    ...
    m_layer_list.emplace_back(
            std::make_unique<ConvolutionLayer>(input_height, input_width, input_channels, filter_height, filter_width,
...
void NeuralNetwork::add_fc_layer(int input_size, int output_size) {
    m_layer_list.emplace_back(
            std::make_unique<FullyConnectedLayer>(input_size, output_size, std::make_unique<TanhFunction>(),
...
void NeuralNetwork::add_output_layer(int input_size, int output_size) {
    m_layer_list.emplace_back(
            std::make_unique<FullyConnectedLayer>(input_size, output_size, std::make_unique<SoftmaxFunction>(),
                                                  std::make_unique<UniformXavierInitialization>(),
                                                  std::make_unique<Adam>(input_size, output_size, output_size)));
}
...
void NeuralNetwork::add_fc_layer_relu(int input_size, int output_size) {
    m_layer_list.emplace_back(
            std::make_unique<FullyConnectedLayer>(input_size, output_size, std::make_unique<ReluFunction>(),
...
void NeuralNetwork::train_batch(Eigen::Ref<const Eigen::MatrixXf> &input_batch,
...
unsigned long NeuralNetwork::layer_size() const {
