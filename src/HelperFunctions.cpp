...
std::string HelperFunctions::get_representation(const Eigen::MatrixXf &input,
                                                int img_height, int img_width,
                                                int num_channels) {
  std::stringstream ss;
  ss << "Matrix of size: " << input.size()
     << "; Number of samples: " << input.cols()
     << "; Images of height: " << img_height << ", width: " << img_width
...
std::string
HelperFunctions::get_comma_representation(const Eigen::MatrixXf &input) {
  std::stringstream ss;
...
std::string
HelperFunctions::get_representation_first(const Eigen::MatrixXf &input,
                                          int img_height, int img_width,
                                          int num_channels) {
  std::stringstream ss;
  ss << "Matrix of size: " << input.size()
...
void HelperFunctions::init_loggers() {
  if (!spdlog::get("convlayer")) {
    try {
      ...
      auto nn_logger = std::make_shared<spdlog::async_logger>(
          "nn", begin(sinks), end(sinks), spdlog::thread_pool(),
          spdlog::async_overflow_policy::block);
      auto convlayer_logger = std::make_shared<spdlog::async_logger>(
          "convlayer", begin(sinks), end(sinks), spdlog::thread_pool(),
          spdlog::async_overflow_policy::block);
